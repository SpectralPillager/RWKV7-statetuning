...
class RWKV_x070(MyModule):
    ...
    # ============ CUDA Graph Support ============
    def setup_cuda_graph(self, state):
        """Setup CUDA Graph for single-token inference. Call after priming with prompt."""
        self._graph_state = state
        self._graph_input = torch.zeros((self.n_embd,), dtype=DTYPE, device="cuda")
        self._graph_output = torch.zeros((self.args.vocab_size,), dtype=DTYPE, device="cuda")

        # Warm up runs (required for CUDA Graph)
        s = torch.cuda.Stream()
        s.wait_stream(torch.cuda.current_stream())
        with torch.cuda.stream(s):
            for _ in range(3):
                self._graph_output.copy_(self.forward_one_alt(self._graph_input, self._graph_state))
    ...
    def forward_one_graph(self, token_id: int):
        """Run single-token inference using captured CUDA Graph. Much faster than forward_batch."""
        self._graph_input.copy_(self.z['emb.weight'][token_id])
        self._graph.replay()
        return self._graph_output
    # ============ End CUDA Graph Support ============
